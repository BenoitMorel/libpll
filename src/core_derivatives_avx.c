...
static int pll_core_update_sumtable_repeats_bclv_4x4_avx(unsigned int sites,
                                           unsigned int rate_cats,
                                           const double * clvp,
                                           ...
                                           double * bclv_buffer)
{
  ...
  for (n = 0; n < sites; n++)
  {
    ...
    lbclv = &bclv_buffer[(parent_site_id[n] - 1) * span_padded];
  ...
  for (n = 0; n < sites; n++)
  {
    ...
    for (i = 0; i < rate_cats; ++i)
    {
      for (j = 0; j < states_padded; j += 4)
      {
        ...
        __m256d xmm1 = _mm256_unpacklo_pd (v_righterm0, v_righterm1);
        __m256d xmm2 = _mm256_unpackhi_pd (v_righterm2, v_righterm3);
        __m256d xmm3 = _mm256_unpacklo_pd (v_righterm2, v_righterm3);
        xmm0 = _mm256_add_pd (xmm0, xmm1);
        xmm1 = _mm256_add_pd (xmm2, xmm3);
        xmm2 = _mm256_permute2f128_pd (xmm0, xmm1, _MM_SHUFFLE(0, 2, 0, 1));
        xmm3 = _mm256_blend_pd (xmm0, xmm1, 12);
